...
def post_reservation(request):
	...
	if form.is_valid():
		...
		res = Reservation.objects.get_or_create(room=room_obj, user=request.user.userprofile, description=description, start_time=res_start_time, end_time=res_end_time)[0]
